  ...
  try {
    ...
    if (!result.success) {
      return json<{ errors: LoginErrors }>(
        { errors: result.error.flatten().fieldErrors as unknown as LoginErrors },
        { status: 400 },
      );
    }
    ...
    // Use boltKV, fallback to mockKV if not available
    ...
    const userData = (await kvStore.get(userKey, { type: 'json' as any })) as { password: string; id: string } | null;
          ...
          <Form method="post" className="mt-8 space-y-6">
              ...
              <div>
                ...
                <div className="mt-1">
                  ...
                  {actionData?.errors?.password ? (
                    <div className="pt-1 text-red-500 dark:text-red-400 text-sm">{actionData.errors.password[0]}</div>
                  ) : null}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-center">
              <div className="text-sm">
                <Link
                  to="/register"
                  className="font-medium text-bolt-elements-accent hover:text-bolt-elements-accent/90"
                >
